...
import { analyzeCSharpFile } from './ast-visitor';
...
async function handleAnalyzeAction(channel: amqplib.Channel, filePath: string): Promise<void> {
  try {
    ...
    // Analyze the file
    const [nodes, relationships] = await analyzeCSharpFile(filePath, idServiceClient);

    // Format the results
    const payload = formatAnalysisResults(filePath, nodes, relationships);
