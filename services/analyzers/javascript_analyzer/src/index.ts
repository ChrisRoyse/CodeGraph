/**
 * JavaScript/TypeScript Analyzer Service for CodeGraph
 * 
 * This service analyzes JavaScript and TypeScript files and extracts code structure information.
 * It connects to the ID Service to generate canonical IDs and GIDs for JS/TS entities.
 */
...
import { analyzeJsFile } from './ast-visitor';
...
export function createAnalysisNodeStubs(nodes: any[]): AnalysisNodeStub[] {
  ...
  for (const node of nodes) {
    ...
    const nodeStub: AnalysisNodeStub = {
      ...
      name: node.name,
      file_path: node.path,
      language: node.path.endsWith('.ts') || node.path.endsWith('.tsx') ? 'typescript' : 'javascript',
      labels,
      properties
    };
...
async function processMessage(
  ...
  try {
    ...
    const isJsTs = filePath.endsWith('.js') || 
                  filePath.endsWith('.jsx') || 
                  filePath.endsWith('.ts') || 
                  filePath.endsWith('.tsx');
    
    if (!isJsTs) {
      logger.info(`Skipping non-JS/TS file: ${filePath}`);
      channel.ack(msg);
      return;
    }
    ...
    try {
      // Analyze the file
      const [nodes, relationships] = await analyzeJsFile(filePath, idServiceClient);
      ...
      // Use the existing channel to publish results
      await channel.publish( // Use existing channel
        '',  // Default exchange
      ...
      // No need to close a separate channel
...
/**
 * Main entry point for the JavaScript/TypeScript Analyzer service
 */
async function main(): Promise<number> {
  try {
    ...
    logger.info(`JavaScript/TypeScript Analyzer started, consuming from ${RABBITMQ_JOBS_QUEUE}`);
    
    // Handle graceful shutdown
    process.on('SIGINT', async () => {
      logger.info('Shutting down JavaScript/TypeScript Analyzer...');
      await channel.close();
