/**
 * Rust Analyzer Service
 *
 * This service analyzes Rust source code files and extracts code structure
...
import { analyzeRustFile } from './ast-visitor';
...
/**
 * Main function to start the Rust Analyzer service
 */
async function main() {
  try {
    logger.info('Starting Rust Analyzer service');
    ...
    await channel.consume(RABBITMQ_JOBS_QUEUE, async (msg) => {
      ...
      try {
        ...
        if (!file_path.match(/\.rs$/)) {
          logger.info(`Skipping non-Rust file: ${file_path}`);
          channel.ack(msg);
          return;
        }
    ...
    logger.info('Rust Analyzer service is running');
  } catch (error) {
    logger.error(`Error starting Rust Analyzer service: ${error}`);
    process.exit(1);
  }
}
...
async function handleAnalyzeAction(channel: amqplib.Channel, filePath: string): Promise<void> {
  try {
    ...
    logger.info(`Analyzing Rust file: ${filePath}`);

    // Analyze the file
    const [nodes, relationships] = await analyzeRustFile(filePath, idServiceClient);

    // Format the results
    const payload = formatAnalysisResults(filePath, nodes, relationships, 'rust');
